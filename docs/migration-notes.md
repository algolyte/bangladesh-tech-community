...
README entries were migrated into `profiles/*.yml` as the source of truth.

## Preserved With Notes

- `profiles/emarat.yml`: the legacy README duplicated the `Skills` label inside the current role line. The migrated profile keeps the clear role and skills, but drops the malformed duplicate markdown.
...
- `profiles/imran110219.yml`: the legacy README showed `github.com/sadmansobhan` as the label but linked to `https://github.com/imran110219`. The migration preserved the URL target because slug generation must follow the real GitHub username.
...
- No availability values were inferred from the legacy README, so migrated profiles default to `unknown`.
